...
public class TopTransactionTest extends BaseIntegrationTest {
  ...
  public void givenValidToken_returnOverallDebitTransaction() throws Exception {
    ...
    mockMvc.perform(
            get(ApiPath.TRANSACTION + ApiPath.TOP_TRANSACTIONS_PER_USER)
        ...
        .andExpect(jsonPath("$[0].amount").value(new BigDecimal("-10.0")))
        .andExpect(jsonPath("$[1].username").value(userTo.getUsername()))
        .andExpect(jsonPath("$[1].amount").value(new BigDecimal("-10.0")))
        .andExpect(jsonPath("$[2].username").value(userTo.getUsername()))
        .andExpect(jsonPath("$[2].amount").value(new BigDecimal("10.0")));
  }
  ...
  private void prepareHistory(User from, User to) {
    ...
    TransactionHistory debit3 = TransactionHistory.builder()
        .amount(BigDecimal.TEN)
        .externalId(externalId)
        .type(CommonConstant.DEBIT)
        .username(to.getUsername())
    ...
    TransactionHistory credit3 = TransactionHistory.builder()
        .amount(BigDecimal.TEN)
        .externalId(externalId)
        .type(CommonConstant.CREDIT)
        .username(from.getUsername())
        .build();
    debit3.setTransactionHistories(credit3);
    credit3.setRelatedTransactionId(debit3);

    transactionHistoryRepository.saveAll(Arrays.asList(debit3, credit3));
  }
